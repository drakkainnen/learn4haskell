...
-}
isVowel :: Char -> Bool
isVowel c = elem c "euioa" || elem c "EUIOA"
...
sumLast2 :: Int -> Int
sumLast2 n = 
  let lastDigitN = lastDigit n
      secondN = lastDigit (div (abs n) 10)
  in lastDigitN + secondN
...
firstDigit :: Int -> Int
firstDigit n = divideEnough (abs n)
  where
    divideEnough :: Int -> Int
    divideEnough x = if x < 10 then x else divideEnough (div x 10)
