...
firstHalf :: [a] -> [a]
firstHalf l = take (ceiling (fromIntegral ((length l)) / 2)) l
...
duplicate :: [a] -> [a]
duplicate [] = []
duplicate (h : tail) = h : h : (duplicate tail)
...
-}
takeEven :: [a] -> [a]
takeEven (_ : x : tail) = x : takeEven tail
takeEven _ = []
...
contains e list = filter (\sl -> elem e sl) list
